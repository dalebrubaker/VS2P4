...
namespace BruSoft.VS2P4.UnitTests
{
    ...
    public class SccProviderServiceTest
    {
        ...
        public static void MyClassInitialize(TestContext testContext)
        {
            ...
            P4OptionsDefaultsProvider   defaults = new P4OptionsDefaultsProvider(new List<string> {"SccProviderServiceTest", "UnitTestDefaults"});
        ...
        {
            Trace.WriteLine(String.Format("Beginning test {0}", _testContext.TestName));
            Trace.Indent();
            _map = new Map();
            SccProviderService sccProviderService = GetSccProviderServiceInstance;
            sccProviderService.P4Service = new P4Service(settings.PerforceServer, settings.PerforceUser, P4ServiceTest.PASSWORD, settings.PerforceWorkspace, false, null, _map);
            sccProviderService.Options = new P4Options(settings);
            sccProviderService.Options.Password = P4ServiceTest.PASSWORD;
        }
        ...
        public void TestFileStateCheckedOutAndAdd()
        {
            ...
            string pendingAddFile = P4ServiceTest.GetTempFileName(settings.WorkspacePath);
        ...
        private MockIVsProject SetUpTestSolution(out IList<string> files, out SccProviderService target, out VsStateIcon[] rgsiGlyphs, out uint[] rgdwSccStatus, out VsStateIcon[] rgsiGlyphsFromStatus)
        {
            ...
            solution.SolutionFile = P4ServiceTest.GetTempFileName(settings.WorkspacePath);
            string projectName = P4ServiceTest.GetTempFileName(settings.WorkspacePath);
            MockIVsProject project = new MockIVsProject(projectName);
            string fileName1 = P4ServiceTest.GetTempFileName(settings.WorkspacePath);
        ...
        public void TestTPDEventsRename()
        {
            ...
            string fileName = P4ServiceTest.GetTempFileName(settings.WorkspacePath);
            string message;
        ...
        private void ResetTestSettings(SccProviderService target)
        {
            ...
            target.Options.Password = P4ServiceTest.PASSWORD;

            // Redefine the P4Service to use our test parameters.
            target.P4Service = new P4Service(settings.PerforceServer, settings.PerforceUser, P4ServiceTest.PASSWORD, settings.PerforceWorkspace,
                                             false, settings.WorkspacePath, new Map());
        }
        ...
        public void CommandEligibilityTests()
        {
            ...
            fileName = P4ServiceTest.GetTempFileName(settings.WorkspacePath);
        ...
        public void SolutionOpenTest()
        {
            ...
            // Wait for the cache to finish updating by calling IsCacheActive
            Assert.IsTrue(target.IsCacheActive(), "Cache should be active.");
            foreach (string file in files)
            {
