...
namespace BruSoft.VS2P4
{
    ...
    public class P4Cache
    {
        ...
        private static System.Diagnostics.TraceSwitch _traceSwitch = new System.Diagnostics.TraceSwitch("P4CacheTracing", "Tracing switch controlling the level of tracing in P4Cache");
        ...
        /// <summary>
        /// IsCacheActive checks whether the cache is being used or not to cache file states.  If the cache is
        ...
        public void AddOrUpdateFilesBackground(VsSelection vsSelection)
        {
            ...
            lock (_fileStatesLock)
            {
                Trace.WriteLineIf(_traceSwitch.TraceVerbose, "P4Cache.AddOrUpdateFilesBackground locked _fileStatesLock");
                foreach (var fileName in fileNames)
                {
                    Trace.WriteLineIf(_traceSwitch.TraceVerbose, String.Format("P4Cache.AddOrUpdateFilesBackground received filename {0}", fileName));
                    if (_fileStates.ContainsKey(fileName))
                    {
