...
namespace BruSoft.VS2P4
{
        ...
        IOleCommandTarget
    {
        ...
        private System.Diagnostics.TraceSwitch _commandTraceSwitch = new TraceSwitch("VS2P4PackageCommandTracing", "Trace command enabling in VS2P4Package");
        ...
        internal VsSelection GetSelection(IList<VSITEMSELECTION> nodes)
        {
            ...
            foreach (VSITEMSELECTION vsItemSel in nodes)
            {
                ...
                {
                    // solution case
                    fileNames.Add(GetSolutionFileName());
                }
                else
                {
        ...
        private string GetProjectFileName(IVsHierarchy pHier)
        {
        ...
        private static IList<uint> GetProjectItems(IVsHierarchy pHier, uint startItemid)
        {
            ...
            while (nodesToWalk.Count > 0)
            {
                ...
                if (pHier.GetProperty(node, (int)__VSHPROPID.VSHPROPID_FirstChild, out firstChildProperty) == VSConstants.S_OK)
                {
                    ...
                    if ((pHier.GetProperty(childnode, (int)__VSHPROPID.VSHPROPID_Expandable, out expandableProperty) == VSConstants.S_OK && (int)expandableProperty != 0) ||
