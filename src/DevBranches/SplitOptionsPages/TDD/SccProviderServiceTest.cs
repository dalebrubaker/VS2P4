...
namespace BruSoft.VS2P4.UnitTests
{
    ...
    public class SccProviderServiceTest
    {
        ...
        {
            _map = new Map();
            SccProviderService sccProviderService = GetSccProviderServiceInstance;
            sccProviderService.P4Service = new P4Service(P4ServiceTest.SERVER, P4ServiceTest.USER, P4ServiceTest.PASSWORD, P4ServiceTest.WORKSPACE, false, null, _map);
#if DEBUG
            sccProviderService.Options = new P4Options(P4ServiceTest.SERVER, P4ServiceTest.USER, P4ServiceTest.PASSWORD, P4ServiceTest.WORKSPACE, Log.Level.Debug);
#else
            sccProviderService.Options = new P4Options(P4ServiceTest.SERVER, P4ServiceTest.USER, P4ServiceTest.PASSWORD, P4ServiceTest.WORKSPACE, Log.Level.Information);
#endif
        }
        ...
        public void TestFileStateCheckedOutAndAdd()
        {
            ...
            string pendingAddFile = P4ServiceTest.GetTempFileName(P4ServiceTest.TESTPATH);
        ...
        private MockIVsProject SetUpTestSolution(out IList<string> files, out SccProviderService target, out VsStateIcon[] rgsiGlyphs, out uint[] rgdwSccStatus, out VsStateIcon[] rgsiGlyphsFromStatus)
        {
            ...
            solution.SolutionFile = P4ServiceTest.GetTempFileName(P4ServiceTest.TESTPATH);
            string projectName = P4ServiceTest.GetTempFileName(P4ServiceTest.TESTPATH);
            MockIVsProject project = new MockIVsProject(projectName);
            string fileName1 = P4ServiceTest.GetTempFileName(P4ServiceTest.TESTPATH);
        ...
        public void TestTPDEventsRename()
        {
            ...
            string fileName = P4ServiceTest.GetTempFileName(P4ServiceTest.TESTPATH);
            string message;
        ...
        public void TestSccMenuCommands()
        {
            ...
            // Redefine the P4Service to use our test parameters (were set to persisted by SetActive();
            target.P4Service = new P4Service(P4ServiceTest.SERVER, P4ServiceTest.USER, P4ServiceTest.PASSWORD, P4ServiceTest.WORKSPACE, false, null, new Map());
        ...
        public void CommandEligibilityTests()
        {
            ...
            Assert.AreEqual(state, FileState.NotInPerforce);
            ...
            Assert.IsFalse(target.IsEligibleForDelete(fileName), "Eligibility error.");
            ...
            fileName = P4ServiceTest.GetTempFileName(P4ServiceTest.TESTPATH);
            ...
            Assert.IsFalse(target.IsEligibleForGetLatestRevision(fileName), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForDelete(fileName), "Eligibility error.");
            Assert.IsFalse(target.IsEligibleForRename(fileName), "Eligibility error.");
            ...
            Assert.IsFalse(target.IsEligibleForDelete(fileName), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForRename(fileName), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForRevisionHistory(fileName), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForDiff(fileName), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForTimeLapse(fileName), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForRevert(fileName), "Eligibility error.");
            ...
            Assert.IsFalse(target.IsEligibleForDelete(sourceFile), "Eligibility error.");
            Assert.IsFalse(target.IsEligibleForRename(sourceFile), "Eligibility error.");
            ...
            Assert.IsFalse(target.IsEligibleForDelete(targetFile), "Eligibility error.");
            Assert.IsTrue(target.IsEligibleForRename(targetFile), "Eligibility error.");
        ...
        public void SolutionOpenTest()
        {
            ...
            foreach (string file in files)
            {
                ...
                Assert.IsTrue(state == FileState.NotSet, String.Format("{0}: Expected file state NotSet before perforce could update, file {1}", state, file));
            }
            VsSelection selection = sccProvider.GetSolutionSelection();
            Assert.IsTrue(selection.FileNames.Count == files.Count);

            // Sleep a second and hope that's long enough for the background thread to get states from Perforce
